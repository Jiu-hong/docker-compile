...
WORKDIR /
RUN git clone -b release-1.5.8 https://github.com/casper-network/casper-node.git 
WORKDIR casper-node
...
FROM scratch
WORKDIR /tmp
COPY --from=stage1 /casper-node/target/release/global-state-update-gen /tmp/global-state-update-gen/1.5.8/