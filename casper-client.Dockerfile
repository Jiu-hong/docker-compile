...
WORKDIR /
RUN git clone -b feat-track-node-2.0 https://github.com/casper-ecosystem/casper-client-rs.git
WORKDIR casper-client-rs
RUN cargo build --release

# stage 2
FROM scratch
COPY --from=0 /casper-client-rs/target/release/casper-client /tmp/casper-client
