...
WORKDIR /
RUN git clone -b feat-2.0 https://github.com/casper-network/casper-sidecar.git
WORKDIR casper-sidecar
...
FROM scratch
COPY --from=0 /casper-sidecar/target/release/casper-sidecar /tmp/casper-sidecar
